...
2. **Credentials stored** — `GET /v2/deployment/{id}` → `credentialsStatus: "stored"` (camelCase on this endpoint; the credentials endpoints return snake_case `credentials_status`). If not, call `POST /v2/deployment/{id}/credentials`.
